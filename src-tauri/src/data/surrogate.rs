...
use rusqlite::{params, types::Value, vtab::array::Array, Connection, Transaction};
...
impl Surrogate {
    ...
    fn build_schema_to_datasource_mapping(
        conn: &Connection,
        schema_oid: i64,
        schema_to_datasource: &mut HashMap<i64, datasource::Datasource>,
    ) -> Result<(), Error> {
    ...
    pub fn get_flat(
        ...
        if base_columns.len() > 1 {
            ...
            for base_column in base_columns {
                ...
                let col = subquery.compile_column(
                    Some(&schema_to_datasource[&base_column.schema.oid]),
                    base_column,
                    expanded_surrogate_table_oid_chain.clone(),
                )?;
            ...
            let Some((subquery_from_clause, _)) = subquery.compile_datasources(false)? else {
                return Err(Error::AdhocError(
                    "Subquery for surrogate key of table failed to compile.",
                ));
            };
            ...
            let col = query.compile_column(
                Some(&schema_to_datasource[&base_columns[0].schema.oid]),
                base_columns[0].clone(),
                expanded_surrogate_table_oid_chain,
            )?;
            Ok(Self {
                oid_expr,
    ...
    pub fn get_object(
        ...
        for result in conn
            ...
            .query_map(params![schema_oid, datasource_alias], |row| {
                Ok((
                    row.get::<_, i64>("OID")?,
        ...
        {
            let (inheritor_schema_oid, inheritor_schema_name, inheritor_schema_datasource_path) =
                result?;
            let inheritor_datasource: Datasource = Datasource::from_alias_transact(
                conn,
                format!("{datasource_alias}{inheritor_schema_datasource_path}"),
            )?;
            schema_to_datasource.insert(inheritor_schema_oid, inheritor_datasource);
            inheritor_datasources.push((
                subquery.insert_datasource(&schema_to_datasource[&inheritor_schema_oid])?,
                inheritor_schema_name,
            ));
        }

        // Compile expression to get object type name
        let obj_type_expr: String = match inheritor_datasources
            .into_iter()
            .map(|(inheritor_datasource_alias, inheritor_schema_name)| {
                format!(
        ...
        if base_columns.len() > 0 {
            let mut json_exprs: Vec<String> = Vec::new();
            for base_column in base_columns {
                let mut expanded_surrogate_table_oid_chain: Vec<i64> =
            ...
            let json_expr: String =
                format!("(SELECT '{{' || {obj_type_expr} || ': {{}}}}' {subquery_from_clause})");
            Ok(Self {
                oid_expr,
                label_expr: json_expr.clone(),
                json_expr,
