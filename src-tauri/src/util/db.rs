...
CREATE VIEW METADATA_FILE AS 
    ...
    SELECT
        ...
        'blob' AS FILE_TYPE,
        NULL AS FILE_PATH,
        b.CONTENT AS FILE_CONTENT_BLOB,
        CASE 
            WHEN b.CONTENT IS NULL THEN NULL 
            WHEN LENGTH(b.CONTENT) > 1000000000 THEN FORMAT('%.1f GB', LENGTH(b.CONTENT) * 0.000000001)
            WHEN LENGTH(b.CONTENT) > 1000000 THEN FORMAT('%.1f MB', LENGTH(b.CONTENT) * 0.000001)
            ELSE FORMAT('%.1f KB', LENGTH(b.CONTENT) * 0.001)
        END AS FILE_CONTENT_SIZE
    FROM __METADATA_FILE f 
...
-- __METADATA_TABLE_COLUMNTYPE_SELECT stores all Select column types.
-- Selects are a storage column that selects a single row from another table.
-- This is represented in UI by a dropdown.
CREATE TABLE IF NOT EXISTS __METADATA_TABLE_COLUMNTYPE_SELECT (
    OID INTEGER PRIMARY KEY REFERENCES __METADATA_TABLE_COLUMNTYPE (OID)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    TABLE_OID INTEGER NOT NULL REFERENCES __METADATA_TABLE (OID)
...
CREATE UNIQUE INDEX IF NOT EXISTS __METADATA_TABLE_COLUMNTYPE_SELECT_INDEX_BY_TABLE_OID ON __METADATA_TABLE_COLUMNTYPE_SELECT (TABLE_OID);
...
CREATE VIEW METADATA_TABLE_COLUMNTYPE AS 
    ...
    SELECT 
        ...
        NULL AS REPORT_OID
    FROM __METADATA_TABLE_COLUMNTYPE ct 
    INNER JOIN __METADATA_TABLE_COLUMNTYPE_SELECT cto ON cto.OID = ct.OID 
    INNER JOIN METADATA_TABLE t ON t.OID = cto.TABLE_OID 
    WHERE NOT ct.TRASH
...
CREATE VIEW METADATA_TABLE_COLUMN_PATH AS 
    ...
    SELECT 
        ...
        u.INHERITOR_DATASOURCE_PATH AS DATASOURCE_PATH,
        c.NAME,
        c.COLUMNTYPE_OID,
        c.COLUMNTYPE,
        c.COLUMNTYPE_DEFAULT_VALUE,
    ...
    FROM METADATA_TABLE_INHERITANCE_PATH u 
    INNER JOIN METADATA_TABLE_COLUMN c ON c.TABLE_OID = u.INHERITOR_TABLE_OID
;
