...
fn setup_db_at_path<P: AsRef<Path>>(path: P) -> Result<(), error::Error> {
    ...
    CREATE VIEW IF NOT EXISTS METADATA_SCHEMA_INHERITANCE_VIEW AS 
        WITH RECURSIVE FLATTENING (INHERITOR_SCHEMA_OID, MASTER_SCHEMA_OID, INHERITOR_DATASOURCE_PATH, MASTER_DATASOURCE_PATH, MAX_DEPTH) AS (
            SELECT
                u.INHERITOR_SCHEMA_OID,
                u.MASTER_SCHEMA_OID,
                '_INHERITOR' || u.INHERITOR_SCHEMA_OID AS INHERITOR_DATASOURCE_PATH,
                '_MASTER' || u.MASTER_SCHEMA_OID AS MASTER_DATASOURCE_PATH,
                1 AS MAX_DEPTH
            ...
            SELECT
                s.INHERITOR_SCHEMA_OID,
                u.MASTER_SCHEMA_OID,
                '_INHERITOR' || u.INHERITOR_SCHEMA_OID || s.INHERITOR_DATASOURCE_PATH AS INHERITOR_DATASOURCE_PATH,
                s.MASTER_DATASOURCE_PATH || '_MASTER' || u.MASTER_SCHEMA_OID AS MASTER_DATASOURCE_PATH,
                s.MAX_DEPTH + 1 AS MAX_DEPTH
            FROM FLATTENING s
        ...
        SELECT 
            ...
            MASTER_SCHEMA_OID,
            MIN(INHERITOR_DATASOURCE_PATH) AS INHERITOR_DATASOURCE_PATH,
            MIN(MASTER_DATASOURCE_PATH) AS MASTER_DATASOURCE_PATH,
